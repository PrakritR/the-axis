...
function getCalendlyUrl({ property, room, tourType }) {
  ...
  return `${CALENDLY_URL}?hide_gdpr_banner=1&primary_color=0f172a&a1=${enc(notes)}`
}
...
export default function TourPopup() {
  ...
  const [step, setStep] = useState(1) // 1=tourType, 2=property, 3=room
  ...
  function reset() { setStep(1); setTourType('in-person'); setPropertyId(null); setRoom('') }
  ...
  function pickTime() {
    const url = getCalendlyUrl({ property: selectedProperty, room, tourType })
          ...
          <div>
            ...
            {room && (
              <div className="mt-5 flex items-center gap-3">
                <button type="button" onClick={pickTime}
