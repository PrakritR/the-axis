...
export function PortalNavGlyph({ tabId, className = 'h-5 w-5' }) {
  ...
  switch (id) {
    ...
    case 'leasing':
    case 'leases':
      /* Document with text — lease / contract (distinct from credit-card payments). */
      return (
        <svg {...svgProps}>
    ...
    case 'applications':
      /* Clipboard + checklist — rental application queue (Heroicons clipboard-document-list). */
      return (
        <svg {...svgProps}>
    ...
    case 'workorders':
      /* Wrench + screwdriver — maintenance / repairs (Heroicons wrench-screwdriver). */
      return (
        <svg {...svgProps}>
    ...
    case 'messages':
    case 'inbox':
      /* Conversation threads (Heroicons chat-bubble-left-right). */
      return (
        <svg {...svgProps}>
